    ...
    <section className="py-16 md:py-20 lg:py-24 bg-gray-50 dark:bg-gray-950 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:pl-[16%] lg:pr-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-10 lg:mb-14 items-end">
          <div>
            <p className="bb-light-description font-sans text-sm sm:text-base dark:text-gray-300">
          ...
          <div className="text-left sm:text-right">
            <span className="bb-subtitle tracking-wide uppercase block mb-2 text-gray-600 dark:text-gray-400 font-medium text-xs sm:text-sm">
              Beam & Bloom
            </span>
            <h2 className="bb-title text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-2 sm:mb-4">
              Our Prices
            </h2>
          </div>
        </div>
          ...
          {pricingCategories.map((card, i) => (
            ...
            >
              <div className="our-price-item flex-1 w-full flex flex-col items-center">
                <h3 className="review-title mb-3 sm:mb-4 text-gray-900 dark:text-white text-sm sm:text-base">
                  {card.title}
                </h3>

                {card.subtitle && (
                  <div className="mb-4 sm:mb-6 w-full flex justify-center">
                    <span className="detail inline-block border-b-2 border-blue-400 pb-1 uppercase tracking-wider text-xs sm:text-sm">
                      {card.subtitle}
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-2 sm:space-y-3 mb-6 sm:mb-8 w-full">
                  {card.items.map((item, j) => (
                    <div
                      key={j}
                      className="grid grid-cols-[1fr_auto_auto] items-center text-gray-600 dark:text-gray-400 font-light text-sm md:text-base lg:text-lg gap-1 sm:gap-2"
                    >
                      {item.link ? (
                        <a
                          href={item.link}
                      ...
                      {item.price && (
                        <span className="font-medium text-gray-900 dark:text-white text-right whitespace-nowrap text-xs sm:text-sm md:text-base">
                          {item.price}
                        </span>
                      )}
                    </div>
